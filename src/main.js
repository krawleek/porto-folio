...
projects.forEach((project,index)=>{
 ...
 article.innerHTML=`<div class="project-surface"><div class="project-art" aria-hidden="true"><img src="/assets/${project.art}" alt="" loading="${index<2?'eager':'lazy'}" fetchpriority="${index===0?'high':'auto'}" decoding="async"></div><span class="tag">${project.nda?'NDA':'CONTEST'}</span><div class="project-copy"><div class="project-icon"><img src="/assets/${project.icon}" alt=""></div><h2>${project.title}</h2><p>${project.description}</p></div><a class="project-link" href="#${project.id}" data-project="${index}" aria-label="${project.title}"></a></div>`;
